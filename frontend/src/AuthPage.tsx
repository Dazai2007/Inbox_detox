...
import { useNavigate } from "react-router-dom"; // ✅ Doğru
...
const AuthPage: React.FC = () => {
  ...
  const navigate = useNavigate(); // ✅ Doğru

  // API adresini (http://127.0.0.1:8000) bir çevre değişkeninden (environment variable)
  // almak en iyisidir, ancak şimdilik bu şekilde kalabilir.
  ...
  const handleLoginSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // ✅ Mükemmel düzeltme: URLSearchParams kullanılıyor
    ...
    try {
      const response = await axios.post(
        ...
        urlEncodedData, // Backend'in beklediği format
        {
          headers: {
  ...
  const handleRegisterSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Kayıt işlemi (register) JSON bekliyordu (auth.py'ye göre), bu doğru.
    const payload = {
      email: registerEmail,
