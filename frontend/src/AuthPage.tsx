...
const AuthPage: React.FC = () => {
  ...
  // API Base URL (Render adresin)
  const API_BASE_URL = "https://api.nexivo.it.com"; 
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response) {
         if (err.response.status === 404) {
            ...
            errorMessage = err.response.data.detail || `Giriş sırasında bir hata oluştu (${err.response.status})`;
         }
        ...
        if (axios.isAxiosError(err) && err.response) {
             if (err.response.status === 404) {
                errorMessage = 'API adresi bulunamadı. Lütfen adresi kontrol edin.';
                 ...
                 errorMessage = err.response.data.detail || `Kayıt sırasında bir hata oluştu (${err.response.status})`;
             }
        } else if (err instanceof Error) { 
           errorMessage = err.message;
        }
